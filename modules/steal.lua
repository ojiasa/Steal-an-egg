-- ═══════════════════════════════════════════════════════════════
-- STEAL MODULE v10.6.5 — Flow Forest + Locked Target State
-- ═══════════════════════════════════════════════════════════════
...
-- 🔒 LOCKED TARGET — Giữ target cố định từ chọn đến khi xong
local currentTarget = nil  -- { prompt, pos, scale, map }
...
-- ⭐⭐⭐ BIG EGG — với LOCKED TARGET
local function findBiggestEgg()
    -- 🔒 Nếu có target khóa → trả về ngay, không scan lại
    if currentTarget ~= nil then
        log("🔒 Dùng target đã khóa: " .. tostring(currentTarget.category or "?"))
    ...
    if #candidates == 0 then
        ...
        currentTarget = {
            prompt = best.prompt, pos = best.pos, scale = best.scale,
            map = best.map, category = best.category,
        }
    ...
    currentTarget = {
        ...
        pos = matchedPos,
        scale = matchedCand.scale,
        map = matchedCand.map,
        category = matchedCand.category,
    }

    return matchedPrompt, matchedPos, matchedCand.scale, matchedCand.map
end

-- ⭐⭐⭐ PRIORITY INCOME — với LOCKED TARGET
...
-- ⭐ v10.6.5: goHomeWithRecovery — có LOCKED TARGET
                        ...
                        for i = 1, 3 do
                            ...
                            if h3 then
                                ...
                                if p3 and p3.Parent then
                                    ...
                                    local p3b = findPromptSteal(h3.Position, 120)
                                    if p3b then
                                        firePromptOnce(p3b)
                                        pickupSuccess = true
                                    else
    ...
    for i = 1, config.MAX_FIRES do
        ...
        if h2 then
            ...
            if not p2 or not p2.Parent then
                p2 = findPromptSteal(h2.Position, 200)
            end
            if not p2 then
...
-- ══════════ MAIN LOOP — FLOW FOREST + LOCK TARGET ══════════
        ...
        if not okMove then
            ...
            if prompt and ppos then
                ...
                if gotKnockback then
                    ...
                    -- ⭐ Chọn target (có LOCKED TARGET check bên trong)
                        ...
                        for attempt = 1, config.MAX_RETRY do
                            ...
                            if attempt > 1 and lastTargetPos then
                                ...
                                if p and ppos then
                                    ...
                                    -- 🔒 Update currentTarget pos
                                    if currentTarget then
                                        currentTarget.pos = ppos
                                        currentTarget.prompt = p
                                    end
                                else
                            ...
                            if stolen then
                                ...
                                if deliveryFailed then
                                    ...
                                    consecutiveFails = 0
                                    replacedThisCycle = false
                                    currentTarget = nil  -- 🔓 UNLOCK TARGET
                                    break
                                end
                            else
                                consecutiveFails = consecutiveFails + 1
...
task.spawn(function()
    pcall(function()
        local chatEvents = RS:WaitForChild("DefaultSystemChatEvents", 5) or RS:WaitForChild("DefaultChatSystemChatEvents", 5)
...
function M.stop() isRunning = false; currentTarget = nil; log("■ STOP") end
...
function M.getCurrentTarget() return currentTarget end
function M.clearCurrentTarget() currentTarget = nil; log("🔓 Unlock target") end
